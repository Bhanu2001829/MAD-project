...
public class ViewBranchActivity extends AppCompatActivity {
    ...
    private void createBranchCard(String branchName, String branchEmail, String branchAddress,
        ...
        branchLocationBtn.setOnClickListener(v -> {
            // Copy mapView value to clipboard
            if (mapView != null && !mapView.isEmpty()) {
                android.content.ClipboardManager clipboard =
                        (android.content.ClipboardManager) getSystemService(CLIPBOARD_SERVICE);
                android.content.ClipData clip = android.content.ClipData.newPlainText("Map Location", mapView);
                clipboard.setPrimaryClip(clip);

                // Notify the user that the location has been copied
                Toast.makeText(ViewBranchActivity.this, "Map location copied to clipboard!", Toast.LENGTH_SHORT).show();
                location = mapView;
            } else {
                Toast.makeText(ViewBranchActivity.this, "Map location is unavailable.", Toast.LENGTH_SHORT).show();
            }

            // Navigate to the map view
            Intent intent = new Intent(ViewBranchActivity.this, ViewBranchOnMapActivity.class);
            startActivity(intent);
        });
    ...
    private boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (itemID == R.id.course) {
            Intent intent = new Intent(ViewBranchActivity.this, ViewCourseActivity.class);
            startActivity(intent);
            return true;
        }
        if (itemID == R.id.home) {
            Intent intent = new Intent(ViewBranchActivity.this, AdminHomeActivity.class);
            startActivity(intent);
            return true;
        }
        if (itemID == R.id.branch) {
            Intent intent = new Intent(ViewBranchActivity.this, ViewBranchActivity.class);
            startActivity(intent);
            return true;
        }
        return false;
